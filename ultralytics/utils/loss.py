...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        self.varifocal_loss = VarifocalLoss(
            gamma=getattr(h, "vfl_gamma", 2.0),
            alpha=getattr(h, "vfl_alpha", 0.75),
        )
        self.hyp = h
    ...
    def _compute_scale_weight_xyxy(target_bboxes: torch.Tensor, fg_mask: torch.Tensor, imgsz: torch.Tensor) -> torch.Tensor:
        """Compute scale-aware weight w=exp(-area) using normalized xyxy box area."""
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        loss[1] = self.varifocal_loss(pred_scores, target_scores.to(dtype), target_labels, area_weight) / target_scores_sum
...
class v8OBBLoss(v8DetectionLoss):
    ...
    def _compute_scale_weight_xywhr(target_bboxes: torch.Tensor, fg_mask: torch.Tensor, imgsz: torch.Tensor) -> torch.Tensor:
        """Compute scale-aware weight w=exp(-area) using normalized xywhr box area."""
        area_weight = torch.ones((*target_bboxes.shape[:2], 1), device=target_bboxes.device, dtype=target_bboxes.dtype)
        if fg_mask.any():
